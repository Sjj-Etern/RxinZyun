-- 浜鸿劯璁よ瘉銆佹満鍣ㄤ汉鍙婅嵂鍝侀厤閫佽褰?CREATE TABLE IF NOT EXISTS face_profiles (
  id INT NOT NULL AUTO_INCREMENT,
  user_id INT NOT NULL,
  face_image MEDIUMTEXT NOT NULL COMMENT '鏈湴鎽勫儚澶撮噰闆嗙殑浜鸿劯鍥剧墖锛堟紨绀虹敤閫旓級',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uk_face_profiles_user (user_id),
...
CREATE TABLE IF NOT EXISTS robots (
  ...
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uk_robots_code (code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT IGNORE INTO robots (code, name, status) VALUES
  ('R001', '閰嶉€佹満鍣ㄤ汉 1 鍙?, 'available'),
  ('R002', '閰嶉€佹満鍣ㄤ汉 2 鍙?, 'available');
...
CREATE TABLE IF NOT EXISTS delivery_records (
  ...
  status ENUM('delivering','arrived','unlocked') NOT NULL DEFAULT 'delivering',
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uk_delivery_prescription_item (prescription_item_id),
  KEY idx_delivery_prescription (prescription_id),
  KEY idx_delivery_robot (robot_id),
